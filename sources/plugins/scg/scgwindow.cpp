...
void SCgWindow::exportToPng(int index)
{

    SCgExportImage exportImage;
    QString selectedFilter;

    QString imageName = currentFileName();
    QString fileName = QCoreApplication::applicationDirPath() + "/";
    if (imageName.isEmpty()) {
        fileName.append(QString::QString::number(index));
    }
    else {
    ...
    if (fileName.length() > 0)
    {
        ...
        if (info.suffix().isEmpty()) {
            ...
            // replace suffix if it not in selected filter
            if (info.suffix() != "png")
                fileName = fileName.left(fileName.size() - info.suffix().size()) + "png";
        }
        exportImage.doExport(mScene, fileName);
    }
}
